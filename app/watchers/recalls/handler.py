"""
Recalls Watcher
Polls Health Canada Recalls and Safety Alerts feed (recalls-rappels.canada.ca)
Filters for medical device entries
Emits normalized items to SQS
Tracks last-seen item GUID in DynamoDB to avoid duplicates
"""
import json
import os
...
RECALLS_FEED_URL = "https://recalls-rappels.canada.ca/en/feed/medical-devices-alerts-recalls"
...
def is_medical_device(item):
    haystack = f"{item['title']} {item['description']} {item['category']}".lower()
    keywords = ["medical device", "device", "implant", "diagnostic", "monitor", "imaging"]
...
def lambda_handler(event, context):
    logger.info(f"Recalls watcher invoked")
    last_seen = set(get_last_seen())

    try:
        xml_bytes = fetch_feed(RECALLS_FEED_URL)
    except urllib.error.URLError as e:
        logger.error(f"Failed to fetch recalls feed: {e}")
        return {"status": "error", "error": str(e)}

    items = parse_rss(xml_bytes)
    logger.info(f"Parsed {len(items)} items from feed")

    new_items = [i for i in items if i["guid"] not in last_seen and is_medical_device(i)]
    logger.info(f"New medical device items: {len(new_items)}")
    ...
    logger.info(f"Recalls watcher complete. Emitted {emitted} new items.")
    return {
        "status": "ok",
        "feed_items_total": len(items),
        "new_medical_device_items": len(new_items),
        "emitted": emitted
    }