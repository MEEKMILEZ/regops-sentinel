...
I added a temporary debug endpoint (/debug/secret, since removed) that fetched the Azure OpenAI secret from Secrets Manager and reported metadata: byte length, first 5 chars, last 5 chars, and a flag for whether the first character was a left brace.
...
In Windows PowerShell 5.1, -Encoding UTF8 writes a UTF-8 file with BOM. AWS Secrets Manager stored those three leading bytes verbatim. Python's json.loads rejected the resulting string because BOM is not whitespace and is not a valid JSON token.

PowerShell 7 changed the default. -Encoding UTF8 there means BOM-less. PowerShell 5.1 callers have to fall through to .NET to get a clean file.
...
"First byte: 0x{0:X2} (should be 0x7B for ASCII left brace)" -f $firstByte
```

Expected output: First byte 0x7B. That is a left brace, the start of the JSON object. If you see 0xEF instead, you have a BOM.

After the put-secret-value, I hit /debug/secret again and confirmed:

- secret_length: 229 (down from 232)
- is_valid_json_first_char: true
...
I am not changing the broad except, it is correct for production durability, but for Phase 4 I am adding a CloudWatch metric for worker.classification_failed so the silent-failure mode cannot happen again. If the metric goes nonzero for more than five minutes, an alarm will fire.
