...
| 2026-05-14 | 34 | Captured (Phase 5C shipped, scope-reduced) | Obligations tracker read-only browse view shipped 2026-05-14 evening. Brain `GET /obligations` + `GET /obligations/{id}` endpoints with cross-table JOIN to devices, tenant-scoped from JWT. New RDS `obligations` table with Health Canada CMDR Section 60 + ISO 13485 + FDA UDI schema (obligation_type, frequency, status, regulatory_body, severity_if_missed). 12 seeded obligations across all status states (overdue, due_soon, upcoming, completed) - mix of device-specific and company-wide. Window `/obligations` page with status-prioritised ordering, smart relative-time due column (5d overdue / In 3d / Today), severity badges. Dashboard `ObligationsDue` widget rewired from stub data to real BFF fetch in same sprint. Bonus: full-text search shipped concurrently using Postgres tsvector + GIN indexes + auto-update triggers across alerts/devices/obligations with ranked UNION. Create/edit UI deferred to Phase 5C.2. |
| 2026-05-14 | 35 | Captured (Phase 5B shipped, scope-reduced) | Device catalog read-only browse view shipped 2026-05-14 evening. Brain `GET /devices` + `GET /devices/{id}` endpoints, tenant-scoped, joined the existing /alerts pattern. New RDS `devices` table with FDA UDI + Health Canada MDL schema, seeded with 12 representative devices for Acme MedDev. Window `/devices` page + BFF proxy + sidebar nav entry. CSV upload + edit flows deferred to Phase 5B.2 - manifest screenshot description amended from "upload screen" to "browse view" to reflect what shipped vs original scope. |
...
| 2026-05-15 | n/a | Phase 5D shipped | Audit bucket hardening complete. (1) Flipped force_destroy = false on aws_s3_bucket.audit so terraform destroy cannot nuke the audit log. (2) Enabled S3 Object Lock on the existing bucket via aws s3api put-object-lock-configuration (out-of-band to avoid the Terraform provider bug that forces bucket replacement on existing buckets). (3) Added aws_s3_bucket_object_lock_configuration resource with GOVERNANCE mode + 1-day default retention managed by Terraform going forward. (4) Backfilled all 84 existing audit blobs with put-object-retention to bring historical records under the same protection. Verified end-to-end: versioned delete on a locked object returns AccessDenied with explicit "protected by object lock" message. Production deployment would use COMPLIANCE mode + the customer's required retention (typically 7 years FDA / 6 years CMDR Section 60). |
| 2026-05-15 | 44 | Captured (deferral resolved) | CloudWatch dashboard `regops-sentinel-dev-ops-1a8df723` captured with live traffic from the day's deploys. The 2026-05-14 manifest amendment had deferred this row pending meaningful traffic; the Phase 6A deployment + Phase 7 digest invocations + Phase 9 OIDC auto-deploy generated enough activity that all 13 widgets show real lines instead of flat zeros. Industry-standard dashboard pattern: ALB request volume + 5xx rate, ECS task count + CPU/memory, RDS CPU/connections/storage, SQS message age + DLQ count, watcher Lambda invocations + errors. |
| 2026-05-15 | 45 | Captured | Phase 6A CloudWatch alarm shown in detail view with metric chart, threshold line, and SNS action wiring. Project ships 10 alarms total: 6 metric alarms (ALB 5xx rate, ALB p99 latency, ECS no-tasks, ECS service unhealthy, RDS CPU sustained, RDS connections high) + 4 composite alarms aggregating related signals. All notifications fan out through `regops-sentinel-dev-ops-alerts-1a8df723` SNS topic. Alarm chosen for capture is `regops-sentinel-dev-ecs-brain-no-tasks-1a8df723` in OK state - the most representative "service is healthy" alarm. |
| 2026-05-15 | 46 | Deferred (capture only) | X-Ray distributed tracing infrastructure deployed (Phase 6B, commits a49b3ae + 10a6131). Brain instrumented with OpenTelemetry SDK + auto-instrumentors for FastAPI, botocore, psycopg, and httpx. ADOT (AWS Distro for OpenTelemetry) Collector sidecar deployed to the brain ECS task with a custom YAML pipeline routing OTLP to AWS X-Ray (the default ECS ADOT config silently routes OTLP to metrics-only, documented in commit 10a6131). Brain logs show "OpenTelemetry instrumentation initialized" on startup; ADOT logs confirm OTLP receivers correctly bound to the traces pipeline. However, X-Ray's get-trace-summaries API returns zero traces after 20+ test requests, indicating spans are generated by the SDK but not arriving at the collector or being silently dropped. Three hypotheses queued for the next debugging session: (1) OTLP/HTTP endpoint path - SDK should auto-append `/v1/traces` but some versions require the full path explicitly; (2) sampler default - opentelemetry-distro's parent-based sampler may filter 100% of inbound HTTP from the ALB; (3) BatchSpanProcessor never flushing - spans queued in memory but the export thread failing silently. Capture deferred until trace pipeline is debugged end-to-end. |
| 2026-05-15 | 48 | Captured | Weekly digest email captured in Hotmail showing real classified regulatory data from Phase 7's first end-to-end run. Email rendered correctly in Outlook/Hotmail with the inline-styles approach (the documented best practice for transactional email; `<style>` blocks are stripped or sandboxed differently across Gmail/Outlook/Hotmail). Email contents: alert count table by urgency (CRITICAL/HIGH/MEDIUM/LOW), top 5 critical/high alerts with title + source + URL, overdue obligations section with red severity badges, upcoming-14-day obligations section. Pattern: EventBridge cron rule â†’ Lambda â†’ RDS query â†’ SES SendEmail. SES sandbox mode means recipient and sender must both be verified; both default to `paschalnnenna@hotmail.com` (handler.py supports comma-separated `DIGEST_RECIPIENTS` env var for future expansion). |
| 2026-05-15 | 49 | Captured | Phase 9 GitHub Actions workflow capture. `brain-ci.yml` run #3 (commit `7710eef`) showing both jobs successful: `Lint + security audit + compile + docker build` (47s, runs on every push) and `Push image and roll ECS service` (3m 26s, only on push to main). Total run 4m 23s end-to-end. The deploy job's steps - OIDC handshake via aws-actions/configure-aws-credentials@v4, ECR login, docker build/tag/push (both `sha-<commit>` and `:latest` tags), ECS `update-service --force-new-deployment`, `wait services-stable`, and a 5-retry curl smoke test against `/health` - exercise the complete industry-standard keyless deploy pattern with no long-lived AWS credentials anywhere in the repo. The one annotation warning is a Node.js 20 deprecation notice on aws-actions/configure-aws-credentials@v4 (not actionable today; will resolve when v5 ships). |
| 2026-05-15 | 50 | Captured | GitHub repo home page captured showing the rendered README with three CI workflow badges (Brain CI, Window CI, Terraform CI - all green), the Phase 9-current status line, and the file browser table. README rewritten earlier today to reflect Phase 6A/6B/7/9 progress: status line updated to "Through Phase 9 (CI/CD auto-deploy)", phase status table expanded to break out Phase 6A (Complete) from Phase 6B (Partial - X-Ray pending), Phase 7 marked Complete, Phase 8 marked Complete, Phase 9 added as Complete; the obsolete CI/CD bullet in "What's not done" replaced with the X-Ray pending bullet; new "Keyless deploys via OIDC" entry added to "Engineering decisions worth flagging"; screenshot count updated from 38 of 50 to 48 of 50. |
